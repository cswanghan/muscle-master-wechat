...
**主包体积**（2026-09-14 已处理）：曾涨到 2212 KB、超上限 164 KB，上传会被拒。
两件事一起做掉：员工端拆成分包（`subPackages` root=`pages/staff`，移出 200 KB，
并从「我的」页 `preloadRule` 预载），图片按质量 75 / 最长边 800px 重压
（1728→1204 KB，这些图最宽只渲染到 750rpx，960px 是白扛的）。
现在主包 **1488 KB**，余量 560 KB。再加图前先看一眼这个数。
...
| 0.17.0 | 2026-08-27 | 技师端业绩明细（按日 / 周 / 月）；等级晋升待确认 |
| 0.18.0 | 2026-08-28 | 储值卡：余额自动抵扣、不够的走微信；「我的」页显示真实余额与流水 |
| 0.19.0 | 2026-09-14 | 会员制课包：老师端「我的会员」「当日时间表」「会员档案」；员工端拆分包，主包 2212→1488 KB |
| 0.20.0 | 2026-09-14 | 按 xmind 补齐 46 条：回访、打卡、成果台与全国排名、客户端课程/打卡、财务六表 |
...
名单集中在 `server/.../catalog/DemoFixtures.java`，catalog、staff_user、床位、
两处 slot 种子全部由它派生。改名单只改这一个文件，测试断言也从它推导。
