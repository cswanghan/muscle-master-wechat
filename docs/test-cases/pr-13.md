...
- **步骤**：微信 `extendOwn` 后 `fire(ADD_ON_PAY_TIMEOUT)`（Job 上下文）。再：现金加钟后再微信加钟超时。
- **预期**：尾格 FREE，原 BUFFER 恢复且 `hold_id` 回主锁；`add_on_hold_id=NULL`；**仅**未支付 ADD_ON 行删除（`start_slot_no >= oldEnd-B`）；已支付现金加钟行保留。订单仍 `IN_SERVICE`。`ReleaseAddOnHold` 不 `fire()`。
- **实际结果**：PASS。`ExtendOwnServiceTest.addOnPayTimeoutRestoresBuffer` / `releaseAddOnHoldKeepsPaidCashItem`。
...
- **步骤**：`payChannel=WECHAT` 加钟 → 拿 `codeUrl` → poll PENDING → mock notify（金额=加钟价）→ 再 poll。同一 `requestId` 重放。过期 PENDING 后再打同 `requestId`，notify 旧码。
- **预期**：先 `LOCKED` 且 `end_slot_no` 不变。notify 后 `confirmPaidAddOn` + `fire(ADD_ON)`，不 `PAY_SUCCESS`。订单 `IN_SERVICE`，`end_slot_no` +M，`add_on_hold_id` 清空。poll SUCCESS。重放仍返回同一 `paymentNo`/`codeUrl`。过期旧码 notify 入退款队列，**不** confirm hold。
- **实际结果**：PASS。`FrontDeskAddOnApiTest.wechatAddOnQrThenNotify` / `wechatAddOnReplayReturnsQr` / `supersededAddOnQrDoesNotConfirmHold`。
...
- **实际结果**：PASS。`ExtendOwn*Test,AddOn*,FrontDeskAddOn*,SlotScan*` 全绿。SlotScan 过期加钟 hold `fire(ADD_ON_PAY_TIMEOUT)`，40904 记 DONE。
