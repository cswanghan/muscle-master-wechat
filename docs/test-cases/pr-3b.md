...
## TC-3b-01 occupySpec + 末格 BUFFER

- **步骤**：`OccupySpecTest`；`SlotOccupyServiceTest.lockNewWritesLastBufferOthersLockedAndInsertsOccupancyAndJob`。
- **预期**：`N = ceil((60+15)/15) = 5`，`buffer_slots=1`；slot 78–81 `LOCKED`，82 `BUFFER`；`hold_id`/`order_id` 在占格前分配；同 TX 写 `booking_order` + `order_item` + `delayed_job(RELEASE_LOCK, biz_key=hold:{holdId}, run_at=now+15min)`；occupancy 10 行（技师 5 + 床 5）。
- **实际结果**：PASS。
...
- **实际结果**：PASS。`skipsBusyBedAndDoesNotLockIt`。技师 REST → `40901` 且回滚；两床皆忙 → `40902` 且技师格仍 FREE。
...
- **实际结果**：PASS。Surefire：`Tests run: 78, Failures: 0, Errors: 0, Skipped: 0`。本 PR 新增 lockNew 相关 14。
