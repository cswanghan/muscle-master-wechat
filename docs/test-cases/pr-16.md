...
- **预期**：先 `workflow=WAIT_APPROVAL`、`refund=WAIT_APPROVAL`、无微信调用、`task_type=REFUND_APPROVE`。审批后微信退款成功，任务 DONE。`refund:approve`。
- **实际结果**：PASS。`RefundTest.amountAtLeast50000WaitsApproval`；`HumanTaskApproveApiTest.amountAtLeast50000WaitsUntilApprove`。

## TC-16-05 同一 `refund_no` / `requestId` 重放幂等

- **步骤**：同一 `requestId` 退两次；再换 `requestId` 打一次。
- **预期**：第二次 `replay=true`、同一 `refund_no`、支付行不增加；微信不重打。
- **实际结果**：PASS。`RefundTest.sameRequestIdAndRefundNoReplayIsIdempotent`；`FrontDeskRefundApiTest.sameRequestIdReplayIsIdempotent`。
...
- **预期**：ALL PASS；可见按 payment 退款、¥500 审批、现金、失败进人工。
- **实际结果**：PASS。
