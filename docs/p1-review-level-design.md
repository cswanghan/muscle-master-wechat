...
> **落地差异（2026-08-25 修订）**：本文写在评价体系独立开发时。实际入 main 的评价本体是
> 另一套实现 —— 表叫 `review`（V6），字段比这里的 `order_review` 少：没有 `positive`、
> `anonymous`、`status`，删除走 `deleted_at` 逻辑删除。因此：
>
> - **好评不再落库定格**，改为每次读时按 `score >= 4` 现算（见 `ReviewPolicy.positive`）。
>   代价是调阈值会连历史一起改写，换来的是不必为一个可随时重算的派生位加一列。
> - **下架评价**由仓储层的 `deleted_at IS NULL` 滤掉，统计计算器根本看不到。
> - 统计物化表 `therapist_stat_30d` 按 §4.2 原样落地，迁移号是 **V7** 不是 V6。
> - §4.1 `order_review` 一节仅作设计留档，**没有实现**；以 V6 的 `review` 表为准。
> - §4.3 的等级表、§5 的 P1/P2 分期均未开工。


> 状态：**统计层已落地**（V7 + 技师卡展示）；评价本体见 V6 `review` 表；等级与价格带未开工
