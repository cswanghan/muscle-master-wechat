...
class TherapistStatCalculatorTest {
    ...
    @Test
    void positiveIsRecomputedFromScoreEveryTime() {
        // review 表不存好评标志，好评与否每次按当前口径（score >= 4）现算。
        // 代价是改阈值会连历史一起改写 —— 这个取舍写在 ReviewPolicy 上。
        ...
        assertThat(stat.positiveCount()).as("3 星不算好评，4 星算").isEqualTo(2);
        assertThat(stat.positiveRateX100()).isEqualTo(6666);
    }

    @Test
    void scoreFourIsTheBoundaryOfPositive() {
        assertThat(ReviewPolicy.positive(ReviewPolicy.POSITIVE_SCORE)).isTrue();
        assertThat(ReviewPolicy.positive(ReviewPolicy.POSITIVE_SCORE - 1)).isFalse();
    }

    @Test
    void therapistWithReviewsButNoVisitsStillGetsARow() {
        // 服务发生在窗口外、评价落在窗口内：两边取并集才不会漏人。
        TherapistStat stat = only(compute(List.of(), List.of(review(1L, T1, 4))));
    ...
    private static List<TherapistStat> compute(List<ServedVisit> visits, List<Review> reviews) {
    ...
    /** 逻辑删除的评价压根到不了这里 —— {@code deleted_at} 在仓储层就滤掉了。 */
    private static Review review(long id, long therapistId, int score) {
        return new Review(id, id, C1, therapistId, 3100L, score,
                null, null, Instant.parse("2026-08-10T03:00:00Z"));
    }
}
